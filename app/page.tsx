  ...
  const columns = [
    ...
    },
    {
      ...
      render: (title: string, row: Product) => {
        ...
        return (
          <a target='_blank' href={`https://admin.shopify.com/store/${vendor}/products/${row.product_id}`}>{title}</a>
        )
      }
    },
    {
      ...
      render: (product_ads_tags: Tag[]) => {
        const PR = product_ads_tags.find((adsTag: Tag) => adsTag.title.indexOf("PR") !== -1)?.title
  ...
  return (
    <div>
      <header className='sticky top-0 z-50 py-4 px-5 shadow-shadow-section bg-white flex justify-between'>
        ...
        />
        <div>
          <span onClick={() => setOpen(!open)} className='cursor-pointer'>Settings</span>
        </div>
      </header>
      <main>
